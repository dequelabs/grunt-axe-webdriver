...
module.exports = function(grunt, WebDriver, Promise, AxeBuilder, reporter) {
	...
	{    
		...
		driver.findElement(webdriver.By.id(this.data.userControlName)).sendKeys(this.data.user);	
		driver.findElement(webdriver.By.id(this.data.passControlName)).sendKeys(this.data.pass);   
		driver.findElement(webdriver.By.className(this.data.loginButtonName)).click();  

		
		console.log('Wainting for ' + this.data.titleToWait);  
		driver.wait(webdriver.until.titleIs(this.data.titleToWait), 1000);
	}
